  ...
  try {
    const r = await fetch(GRAFANA, {
      ...
      body: JSON.stringify({
        queries: [{
          datasource: DS,
          query: `from(bucket: "nautilus") |> range(start: -48h) |> filter(fn: (r) => r["_measurement"] == "sonardyne_nav") |> pivot(rowKey:["_time"], columnKey:["_field"], valueColumn:"_value") |> filter(fn: (r) => r["name"] == "Sntry 5206") |> sort(columns:["_time"]) |> keep(columns:["_time","latitude","longitude","depth"])`,
          refId: 'A'
        }],
        from: 'now-48h',
        to: 'now'
